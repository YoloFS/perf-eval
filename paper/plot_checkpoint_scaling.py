...
def main():
    ...
    out_path = os.path.join(paper_dir, 'checkpoint-scaling-figure.pdf')

    def read_csv(name):
        with open(os.path.join(paper_dir, name)) as f:
            return list(csv.DictReader(f))

    create_rows = read_csv('checkpoint-scaling-create.csv')
    read_rows = read_csv('checkpoint-scaling-read.csv')
    commit_rows = read_csv('checkpoint-scaling-commit.csv')
    ...
    def plot_panel(ax, rows, panel_label, show_ylabel, ylabel=None, exclude_backends=None,
    ...
    plot_panel(ax_create, create_rows, 'create', show_ylabel=True, mark_overlay_failure=True)
    plot_panel(ax_read, read_rows, 'read', show_ylabel=False, mark_overlay_failure=True)
    # Main commit panel: YoloFS and OverlayFS only (linear scale).
    plot_panel(ax_commit, commit_rows, 'commit', show_ylabel=True, ylabel='ms',
    ...
    # Convert commit y-axis from us to ms for readability.
