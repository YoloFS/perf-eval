...
pub struct FioSeqReadCold;

pub fn spec() -> FioSpec {
    FioSpec {
        name: "fio-seq-read-cold",
        rw: "read",
        warm_cache: false,
...
pub fn details() -> workloads::WorkloadDetails {
    workloads::fio_workload_details(
        "Sequential buffered read benchmark with cold page cache.",
        file!(),
        spec(),
    )
}

impl Workload for FioSeqReadCold {
    ...
    fn description(&self) -> &'static str {
        "Sequential 4K read, 1 GB file, cold page cache (fio)"
    }
