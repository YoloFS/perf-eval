use crate::workload::{CacheMode, Workload, WorkloadKind};
use crate::workloads;
use crate::workloads::meta_shared::{self, MetaSource};
use anyhow::Result;
use std::path::Path;

pub struct MetaStatCold {
    pub source: MetaSource,
}

impl MetaStatCold {
    pub fn all() -> Vec<Self> {
        MetaSource::ALL
            .iter()
            .map(|&s| Self { source: s })
            .collect()
    }
}

pub fn details() -> workloads::WorkloadDetails {
    let mut d = workloads::workload_details(
        "One stat syscall after dropping page caches, measuring cold metadata lookup latency.",
        "Fixture varies by source: base populates before mount; stage creates inside the mount; checkpoint snapshots after creation.",
        Some("Page cache is dropped after all subprocess setup but before the timed stat."),
        &meta_shared::meta_stat_cold_execution(),
        file!(),
    );
    d.caveat = Some(meta_shared::COLD_MOUNT_CAVEAT.to_string());
    d
}

impl Workload for MetaStatCold {
    fn name(&self) -> &'static str {
        match self.source {
            MetaSource::Base => "meta-stat-cold-base",
            MetaSource::Stage => "meta-stat-cold-stage",
            MetaSource::Checkpoint => "meta-stat-cold-checkpoint",
        }
    }
    fn kind(&self) -> WorkloadKind {
        WorkloadKind::Op
    }
    fn description(&self) -> &'static str {
        match self.source {
            MetaSource::Base => "One cold stat against a 10,000-file base-layer fixture",
            MetaSource::Stage => "One cold stat against a 10,000-file stage-local fixture",
            MetaSource::Checkpoint => {
                "One cold stat against a 10,000-file checkpoint-layer fixture"
            }
        }
    }
    fn work_dir(&self) -> &'static str {
        self.name()
    }
    fn ensure_fixture(&self) -> Result<()> {
        Ok(())
    }
    fn realistic_rules(&self, session_root: &Path) -> Vec<(String, agfs::config::Perm)> {
        workloads::allow_rw_rules(session_root)
    }
    fn populate_base(&self, base_work_dir: &Path) -> Result<()> {
        meta_shared::populate_files_for_source_cold(self.source, base_work_dir)
    }
    fn prepare_workdir(&self, dest: &Path) -> Result<()> {
        meta_shared::prepare_files_for_source_cold(self.source, dest)
    }
    fn needs_prepare_workdir(&self) -> bool {
        meta_shared::needs_prepare(self.source)
    }
    fn needs_checkpoint(&self) -> bool {
        meta_shared::needs_checkpoint(self.source)
    }
    fn cache_mode(&self) -> CacheMode {
        meta_shared::cache_mode(true)
    }
    fn run(&self, dest: &Path, _verbose: bool) -> Result<()> {
        meta_shared::run_meta_stat_cold(dest)
    }
}
