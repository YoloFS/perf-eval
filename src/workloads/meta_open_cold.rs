...
pub fn details() -> workloads::WorkloadDetails {
    let mut d = workloads::workload_details(
        "One open syscall after dropping page caches, measuring cold path lookup/open latency.",
        "Fixture varies by source: base populates before mount; stage creates inside the mount; checkpoint snapshots after creation.",
        Some("Page cache is dropped after all subprocess setup but before the timed open."),
        &meta_shared::meta_open_cold_execution(),
        file!(),
    );
...
impl Workload for MetaOpenCold {
    fn name(&self) -> &'static str {
        match self.source {
            MetaSource::Base => "meta-open-cold-base",
            MetaSource::Stage => "meta-open-cold-stage",
            MetaSource::Checkpoint => "meta-open-cold-checkpoint",
        }
    }
    ...
    fn description(&self) -> &'static str {
        match self.source {
            MetaSource::Base => "One cold open against a 10,000-file base-layer fixture",
            MetaSource::Stage => "One cold open against a 10,000-file stage-local fixture",
            MetaSource::Checkpoint => {
                "One cold open against a 10,000-file checkpoint-layer fixture"
            }
        }
    }
    ...
    fn populate_base(&self, base_work_dir: &Path) -> Result<()> {
        meta_shared::populate_files_for_source_cold(self.source, base_work_dir)
    }

    fn prepare_workdir(&self, dest: &Path) -> Result<()> {
        meta_shared::prepare_files_for_source_cold(self.source, dest)
    }
    ...
    fn run(&self, dest: &Path, _verbose: bool) -> Result<()> {
        meta_shared::run_meta_open_cold(dest)
    }
}
