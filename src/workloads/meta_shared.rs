...
workloads::define_rust_execution!(
    fn run_meta_append(dest: &Path) -> Result<()> {
    ...
    } => meta_append_execution
);

workloads::define_rust_execution!(
    fn run_meta_rename(dest: &Path) -> Result<()> {
        let mut latencies = Vec::with_capacity(workloads::OP_FILE_COUNT);
        let total = Instant::now();
        ...
        workloads::emit_op_result(&workloads::summarize_latencies(latencies, total.elapsed(), None))
    } => meta_rename_execution
);

workloads::define_rust_execution!(
    fn run_meta_unlink(dest: &Path) -> Result<()> {
        let mut latencies = Vec::with_capacity(workloads::OP_FILE_COUNT);
        let total = Instant::now();
        ...
        workloads::emit_op_result(&workloads::summarize_latencies(latencies, total.elapsed(), None))
    } => meta_unlink_execution
);

workloads::define_rust_execution!(
    fn run_meta_stat_cold(dest: &Path) -> Result<()> {
...
workloads::define_rust_execution!(
    fn run_meta_stat_warm(dest: &Path) -> Result<()> {
        ...
        workloads::emit_op_result(&workloads::summarize_latencies(latencies, total.elapsed(), None))
    } => meta_stat_warm_execution
);

workloads::define_rust_execution!(
    fn run_meta_readdir_cold(dest: &Path) -> Result<()> {
        ...
        workloads::emit_op_result(&workloads::summarize_latencies(latencies, total.elapsed(), None))
    } => meta_readdir_cold_execution
);

workloads::define_rust_execution!(
    fn run_meta_readdir_warm(dest: &Path) -> Result<()> {
